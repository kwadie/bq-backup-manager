...
provider "google" {
  ...
  access_token    = var.terraform_service_account
  request_timeout = "60s"
}
...
locals {
  ...
  cloud_scheduler_account = "service-${data.google_project.project.number}@gcp-sa-cloudscheduler.iam.gserviceaccount.com"
}
